...
- **L3**: 100µH inductor, 3A saturation current rating (same as Diagram2/3)
- **D3**: SS34 Schottky diode, 3A, 40V minimum (catch diode from SW to -13.5V)
- **C9**: 100µF electrolytic, 35V+ rating (input bulk, sees VIN+|VOUT| = 28.5V)
...
2. **Input Voltage Stress**: Input caps see VIN + |VOUT| = 28.5V (use 35V+ rating)
...
- `C9 (100µF electrolytic, 35V)`: Between `+15V input` and `-13.5V output` (IC GND)
