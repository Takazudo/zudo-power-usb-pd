...
-15V ──┬─────────────    ┌──────────────┐
       │                 │              │        L3 (100µH)
       ├─ C9 (100µF)     │  LM2596S-ADJ │    ┌───[inductor]───┬─→ -13.5V OUT
       │     │           │     (U5)     │    │                │
       │    GND          │              │    │                │
       │                 │   1 VIN  OUT 2├───┤                │
       ├─ C10 (100nF)    │              │    │                │
...
Key Connections:
- C9, C10: Input filter capacitors (between -15V and GND)
- D3: Freewheeling diode (cathode to switching node, anode to GND)
- C11: Output filter capacitor (between -13.5V and GND)
- FB network: Voltage divider sets output to -13.5V
```
