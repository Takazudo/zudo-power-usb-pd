...
describe("the corpus normalizes to the figures the epic states", () => {
  ...
  it("counts 126 sources, 460 facts, 145 coverage domains, 36 interactions", () => {
    assert.equal(model.corpus.sources, 126);
    assert.equal(model.corpus.facts, 460);
  ...
  it("publishes every instance without duplicating or dropping one", () => {
    ...
    assert.equal(sum(model.records, (entry) => entry.facts.length), 460);
  ...
  it("gives every record a unique, route-safe slug and page-unique anchors", () => {
    ...
    assert.equal(total, 41 + 126 + 460 + 145 + 39 + 41);
...
describe("the real records the epic calls out", () => {
  ...
  it("publishes open coverage, both with and without applicable blockers", () => {
    ...
    assert.equal(coverage.filter((entry) => entry.status === "COVERED").length, 87);
    ...
    assert.equal(withBlockers.length, 54);
  ...
  it("preserves calculated facts, their expressions and cross-record edges", () => {
    ...
    assert.equal(calculated.length, 49);
    assert.equal(facts.filter((fact) => fact.dependsOn.length > 0).length, 49);
  ...
  it("keeps numeric, string and structured fact values in their own shapes", () => {
    ...
    assert.equal(values.filter((value) => typeof value === "number").length, 246);
    assert.equal(values.filter((value) => typeof value === "string").length, 206);
