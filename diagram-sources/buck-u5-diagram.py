#!/usr/bin/env python3
"""
LM2596S-ADJ Buck Converter: -15V → -13.5V (Negative Voltage)
...
with schemdraw.Drawing(
    ...
    # C14 from junction2 (closer to IC - high-freq decoupling)
    # For negative voltage: ceramic capacitor (non-polarized, no flip needed)
    d.pop()
    elm.Capacitor().down(2.0).label('C14\n100nF', loc='bot')
    elm.Ground()

    # C13 from junction1 (farther from IC - bulk filtering)
    # For negative voltage: electrolytic with reversed polarity
    # Negative terminal (top) to -15V, positive terminal (bottom) to GND
    d.pop()
    elm.Capacitor().down(2.0).reverse().label('C13\n100µF', loc='bot')
    elm.Ground()
    ...
    elm.Inductor().right(2.5).label('L3\n100µH\n4.5A', loc='top',ofst=0.2)
    elm.Dot()
    ...
    # Feedback voltage divider network (R5 || C12 in parallel, then R6)
    elm.Line().up(4)
    elm.Dot()
    d.push()  # Save position before R5 for parallel C12 capacitor

    elm.Resistor(scale=0.7).left().label('R5\n10kΩ', loc='bot', ofst=0.5)
    r5_end = d.here
    d.push()  # Save position after R5 (this is the Tap)

    elm.Dot()  # Tap junction for FB connection
    d.push()  # Save tap position for FB connection

    elm.Resistor(scale=0.7).left().label('R6\n1kΩ', loc='bot', ofst=0.5)
    elm.Line().left(0.2)
    elm.Line().down(0.2)
    elm.Ground()
    ...
    # C12 capacitor in parallel with R5 (feedback compensation)
    d.pop()  # Return to r5_end (after R5)
    d.pop()  # Return to r5_start (the dot junction)
    elm.Line().up(2)
    elm.Capacitor().left().label('C12\n22nF', loc='top', ofst=0.3)
    elm.Line().to(r5_end)

    # Output capacitor C11 (from output junction - right side, facing up)
    # For negative voltage: electrolytic with reversed polarity
    # Negative terminal (top) to -13.5V, positive terminal (bottom) to GND
    d.pop()
    elm.Capacitor().down(2.0).reverse().label('C11\n470µF\n25V', loc='bot', ofst=0.5)
    elm.Ground()

    # Flyback diode D3 (from switching node - left side, facing down)
    # NEGATIVE VOLTAGE: Diode flipped - anode to switching node, cathode to GND
    # Allows current flow: switching node → through diode → GND (when switch OFF)
    d.pop()
    elm.Diode().down(2.0).label('D3\nSS34', loc='top')
    elm.Ground()

    # Save to doc/static/circuits/ (one level up from diagram-sources)
    import os
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_path = os.path.join(script_dir, '../doc/static/circuits/buck-u5-diagram.svg')
    d.save(output_path)
